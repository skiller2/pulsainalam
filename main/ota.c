// levantar server web en mingw32con " python -m SimpleHTTPServer 8070 "
...
{	size_t str_len=255;
	...
	nvs_get_str(handle_config,"ota_uri",ota_uri,&str_len);

    ESP_LOGI(TAG, "URL OTA %s", ota_uri);



	url=parse_url(ota_uri);


    ESP_LOGI(TAG, "Server IP: %s Server Port:%s", url->host, url->port);
...
//    sock_info.sin_addr.s_addr = addr;
...
{	esp_log_level_set( TAG, ESP_LOG_INFO); // ESP_LOG_NONE  ESP_LOG_INFO
    ...
    vTaskDelay(2000); // para que la web temine ante de ota
    ...
    int get_len = asprintf(&http_request, GET_FORMAT, url->path, url->host, url->port);
    if (get_len < 0) {
